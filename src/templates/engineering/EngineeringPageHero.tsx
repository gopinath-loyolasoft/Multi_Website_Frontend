...
export const EngineeringPageHero: React.FC<TemplatePageHeroProps> = ({
  badge,
  title,
  subtitle,
  icon,
  ...
  return (
    <section className={`relative ${engineeringConfig.heroGradient} text-white py-16 md:py-24 px-4 sm:px-6 lg:px-8 overflow-hidden`}>
      ...
      <div className="max-w-4xl mx-auto relative z-10 text-center">
        {badge && (
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-xs font-semibold uppercase tracking-wider text-white mb-4">
            {icon || <Crest className="w-4 h-4 text-blue-400" />}
            <span>{badge}</span>
          </div>
        )}

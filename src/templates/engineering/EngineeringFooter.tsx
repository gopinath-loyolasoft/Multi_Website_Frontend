...
export const EngineeringFooter: React.FC<TemplateFooterProps> = ({ siteConfig }) => {
      ...
      <div className="max-w-screen-2xl mx-auto px-6 sm:px-10 lg:px-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white ${engineeringConfig.crestBg} shadow-md`}>
              <Crest className="w-6 h-6" />
            </div>
            <div>
              <span className={`text-xl text-white block leading-tight ${engineeringConfig.nameFont}`}>{tenant.name}</span>
              <span className="text-xs font-semibold block mt-0.5 text-blue-400">
                {siteConfig.settings?.tagline || engineeringConfig.defaultBadge}
              </span>
            </div>
          </div>

          <p className="text-xs text-slate-400 max-w-sm leading-relaxed">
            {themeConfig?.badgeText || engineeringConfig.footerDescription}
          </p>
        </div>
